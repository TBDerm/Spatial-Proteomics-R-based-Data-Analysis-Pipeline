...
* Input parameters (all input at the top of the page):
  ...
  * Name folder for output datasets to be saved in (automatic)
  * Name an ‘experiment identifier’ – used to append output file names with, to prevent confusion when viewing data later
  * Select type of normalisation wanted
    * No normalisation
  ...
  * Select type of imputation wanted
...
* After this information is input, the code can be run in full, and all output files will be saved to a new folder (named in the parameters).
* Generates venn diagrams of proteins, and peptides, in each experimental group (fig. a)
...
* Filters protein dataset to exclude any proteins with <2 peptides
  * Generates a missing values plot (fig. b) showing proportion of missing values per sample, prior to filtering for missing values
...
* Generates a variety of data visualisations (selection shown in figure) (based on the filtered imputed protein data)
  ...
  * Z-scored heatmap of the top 30 proteins by median intensity value (fig. d)
  * Violin and boxplots to show protein data distribution per sample, divided into experimental groups (fig. h)
  * Frequency distribution curves of protein intensity values, per experimental group and per sample (fig. f) – these allow the user to check that data distribution is normal and similar for each sample/group after normalisation
  * Frequency distribution histograms of protein intensity values, per experimental group (fig. g) – an alternative way of displaying the above
  * Histograms of number of peptides per protein, across all samples and per experimental group (fig. c) – informs the user how many peptides were identified for the majority, or minority, of proteins
* Performs PCA on the imputed protein data
  * Generates a large PCA plot of PC1 and PC2, and a composite of all PC axes combinations up to PC4 (fig. e) – shows any separation of experimental groups, thus attributing a percentage of variance in the data (derived from PCs, hence the different axes display) to differences between these experimental groups
...
* Performs DE analysis on the imputed protein data
  ...
  * Generates volcano plots (fig. i) (one per combination pair of experimental groups, if there are over two), with and without protein gene labels for proteins beyond the significance threshold
...
## Labelled volcanoes scripts
This step in the analysis pipeline is the generation of volcano plots with protein category organisation. This is a unique tool that creates volcano plots based on the DE results produced by the previous step in the pipeline. Volcano plot datapoints are coloured and annotated with labels coloured according to either BMbase categories, matrisomeDB categories, or a combination. This makes it easy to spot trends in the DE analysis, for example a marked increase in BM proteins, seen in the figure below (green and blue datapoints all on the right-hand side of the volcano plot).


<img src="https://github.com/user-attachments/assets/44a9fea0-5ebe-447d-a5c4-aa8568680b3c" width="500"/>

To generate these plots, the user uploads the DE results, produced in the previous part of the pipeline (with protein categories annotated). The experimental group comparison is named (e.g. ‘Epidermis_Intact-Epidermis_Removed’), and the P-adj value and FC value thresholds set. The script then presents a variety of labelling options; to label only BM / ECM proteins, or all, beyond the threshold, plus additional labels for BM / ECM protein labels below the threshold also. Datapoints are all colour-coded.
